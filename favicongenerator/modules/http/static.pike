...
mapping(string:mixed)|Concurrent.Future http_request(Protocols.HTTP.Server.Request req, string filename)
{
	...
	constant path_directory = ([
		"images": "httpstatic/images",
		"images/icons": "httpstatic/images/icons",
		"images/products": "httpstatic/images/products",
		"images/products/temp": "httpstatic/images/products/temp"
	]);

	if (has_value(filename, '/')) {dir = path_directory[dirname(filename)] || dir; filename = basename(filename);}

	//Guess a MIME type based on the extension, by default
	foreach (([".css": "text/css", ".flac": "audio/flac", ".mp3": "audio/mp3", ".html": "text/html", ".js": "text/javascript", ".svg": "image/svg+xml"]); string ext; string t)
	...
	if (dir == "httpstatic" && (has_suffix(filename, ".js") || has_suffix(filename, ".html"))) {
